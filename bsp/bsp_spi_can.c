...
/* CAN 통신 초기화  */
bsp_bool CANSPI_Initialize(void)
{
  RXF0 RXF0reg;
  RXF1 RXF1reg;
  RXF2 RXF2reg;
  ...
  RXF1reg.RXF1SIDL = 0x00; //Starndard Filter
...
/* CAN 메시지 전송 */
bsp_bool CANSPI_Transmit(uCAN_MSG *tempCanMsg)
{
  bsp_bool returnValue = false;
...
/* CAN 메시지 수신 */
bsp_bool CANSPI_Receive(uCAN_MSG *tempCanMsg)
{
  bsp_bool returnValue = false;
  rx_reg_t rxReg = {0};
